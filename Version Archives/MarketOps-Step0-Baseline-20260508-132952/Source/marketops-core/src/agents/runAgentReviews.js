...
const reviewBlueprints = [
  { key: "coordinator", purpose: "Coordinate paper runs, content office outputs, review cadence, and QA boundaries.", worked: "The office run, queue, compliance report, and dashboard refresh are now connected into a repeatable local flow.", failed: "The human review summary needs trend context instead of only latest-run status.", confidence: "Medium-high confidence in run coordination; lower confidence in long-horizon trend interpretation until more history exists." },
...
function reviewFor(blueprint, fileMeta, inputs, proposals) {
  ...
  return {
    ...
    inputsReviewed: ["latest paper run summary", "public dashboard bundle", "office content queue", "content compliance report"],
...
function writeRollups(inputs, reviews, proposals) {
  ...
  const strongest = ["Paper-only guardrails remain intact.", "Content queue stays review-gated.", "Compliance scan passed before agent review." ];
