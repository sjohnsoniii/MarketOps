...
function validateInstalledTask(checks, label, task, expectedScript, expectedSchedule) {
  ...
  add(checks, `${label} task uses approved script`, action.includes(expectedScript), action);
...
function runAutomationCheck() {
  ...
  [["paper PowerShell runner", paths.paperRunner], ["office PowerShell runner", paths.officeRunner], ["paper scheduled-task install helper", paths.paperInstall], ["paper scheduled-task remove helper", paths.paperRemove], ["office scheduled-task install helper", paths.officeInstall], ["office scheduled-task remove helper", paths.officeRemove]].forEach(([label, filePath]) => fileExists(checks, label, filePath));
  ...
  const paperInstallState = validateInstalledTask(checks, "paper", paperTask, "C:\\Users\\sjohn\\Desktop\\Projects\\MarketOps\\Scripts\\run-marketops-paper-full-v0.1.ps1", "paper_30m");
