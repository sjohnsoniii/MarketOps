...
function runOfficeQa() {
  ...
  if (Array.isArray(queue.items) && queue.items.length >= 17) pass(checks, "Content queue contains report, social, video, and avatar drafts", `${queue.items.length} items`);
  else fail(checks, "Content queue contains report, social, video, and avatar drafts", `${Array.isArray(queue.items) ? queue.items.length : 0} items`);
